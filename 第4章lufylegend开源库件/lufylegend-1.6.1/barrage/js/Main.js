...
function onup(event){	monseIsDown = false;
	player.move = [0,0];
	player.canshoot = false;
	player.shootctrl = player.shootspeed;
}
function ondown(event){	monseIsDown = true;	player.shootcount = 0;	player.shootctrl = player.shootspeed+1;	player.canshoot = true;
}
/**
 * 循环
 * */
function onframe(){
	if(gameover){//游戏结束
		...
		if(monseIsDown){		if(player.x + 30 - player.speed > LGlobal.offsetX){//left
						player.move[0] = -1;		}else if(player.x + 30 + player.speed < LGlobal.offsetX){//right
			player.move[0] = 1;		}else{			player.move[0] = 0;		}		if(player.y + 30 - player.speed > LGlobal.offsetY){//up
			player.move[1] = -1;		}else if(player.y + 30 + player.speed < LGlobal.offsetY){//down
			player.move[1] = 1;		}else{			player.move[1] = 0;		}	}	
	var i;
	//循环子弹
